...
class BenchmarkTimer
{
    ...
    public function __construct($callFct = false)
    {
        ...
        echo '<table>';
        echo '<tr>';
        echo '<td>Fonctions </td><td>  temps </td><td> mémoire avant </td><td> mémoire après </td><td> pique </td><td> delta </td>';
        echo '</tr>';
...
//	$memory -= $this->startMemory;
        echo '<tr>';
        echo '<td>' . str_pad($time_itle, 23) . " : </td><td>" . number_format($time, 5) . " sec et </td>";
        $this->totalTime += $time;
        unset($time, $usec, $this->string_ec);
        $delta = $memory - $this->startMemory;
        echo '<td>' . $this->startMemory . '  ';
        echo '<td>' . $memory . ' </td> ';
        echo '<td>' . $pick . " </td> ";
        echo '<td>' . $this->convert($delta) . "</td>";
        echo '</tr>';
