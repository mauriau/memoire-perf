...
function test_math(&$result, $count = 150000) {
    $timeStart = microtime(true);
...
function test_string(&$result, $count = 150000) {
    $timeStart = microtime(true);
...
function test_loops(&$result, $count = 150000) {
    $timeStart = microtime(true);
    for ($i = 0; $i < $count; ++$i) {
...
function test_ifelse(&$result, $count = 150000) {
    $timeStart = microtime(true);
    for ($i = 0; $i < $count; $i++) {
        if ($i == -1) {
...
function test_mysqli(&$result, $settings) {
    $timeStart = microtime(true);
...
function test_mysql(&$result, $settings) {
    $timeStart = microtime(true);
    $link = mysql_connect($settings['db.host'], $settings['db.user'], $settings['db.pw']);
    $result['benchmark']['mysql']['connect'] = timer_diff($timeStart);
    //$arr_return['sysinfo']['mysql_version'] = '';
    mysql_select_db($link, $settings['db.name']);
    $result['benchmark']['mysql']['select_db'] = timer_diff($timeStart);
    $dbResult = mysql_query($link, 'SELECT VERSION() as version;');
    $arr_row = mysql_fetch_array($dbResult);
    $result['sysinfo']['mysql_version'] = $arr_row['version'];
    $result['benchmark']['mysql']['query_version'] = timer_diff($timeStart);
    $query = "SELECT BENCHMARK(1000000,ENCODE('hello',RAND()));";
    $dbResult = mysql_query($link, $query);
    $result['benchmark']['mysql']['query_benchmark'] = timer_diff($timeStart);
    mysql_close($link);
    $result['benchmark']['mysql']['total'] = timer_diff($timeStart);
    return $result;
